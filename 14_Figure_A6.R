...
a <-  m %>%
  group_by(cntry) %>%
  do(model1 = tidy(lm(mainact  ~ parents_occup + gndr + age + education*parents_occup + education* crisis + parents_occup*crisis + data, data = .),  conf.int=TRUE)) %>%   ## Same as question
  gather(model_name, model, -cntry) %>%                        ## make it long format
  unnest() 

#Prepare the data to plot it. We filter the interaction of interest, that is parental_occupation*crisis.
b = filter(a,term=="parents_occupWorking class:crisisPost-recession")
...
g1= ggplot(data= b,mapping = aes(estimate,cntry)) + 
  ...
  scale_x_continuous(limits = c(-0.2, 0.25), breaks = seq(-0.2, 0.2, by = 0.1),labels = scales::percent_format(accuracy = 1))+
...
z <-  m %>%
  group_by(cntry) %>%
  do(model1 = tidy(lm(ln_wage  ~ parents_occup + gndr + age + education*parents_occup + education*crisis + parents_occup*crisis , data = .),  conf.int=TRUE)) %>%   ## Same as question
  gather(model_name, model, -cntry) %>%                        ## make it long format
  unnest() 

#Fix the country order of appearance
z =  z %>% mutate(cntry = fct_relevel(cntry,"United Kingdom", "Spain", "Poland", "Italy", "Germany", "France")) 

#Prepare the data to plot it. We filter the interaction of interest, that is parental_education*crisis.
q= z %>% filter(term=="parents_occupWorking class:crisisPost-recession")
...
ggsave("Figure_A6.pdf", scale=1.35, width = 10, height = 7, dpi= 2300,limitsize = FALSE,units = c("in"))
