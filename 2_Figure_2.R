...
m2 = m %>%
  ...
  summarise(emp = round(mean(mainact,na.rm = T),3))%>%
  ungroup()  
m2 <- na.omit(m2)
m2$emp =round(m2$emp*100)
m2$diff = m2$emp[m2$crisis == "Post-recession"] - m2$emp
...
m2 %>% filter(crisis=="Prior-recession")%>%
  ...
  geom_text(aes(y = diff + ifelse(diff>=0,0.01, -1.2),label = diff), vjust =  -0.5, colour = "black",position = position_dodge(.9), size =6)+
...
#Save the figure
ggsave("Figure_2.pdf", scale=1.35, width = 12, height = 7, dpi= 2300,limitsize = FALSE,units = c("in"))
