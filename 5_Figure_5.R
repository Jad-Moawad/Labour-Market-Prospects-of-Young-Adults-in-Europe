...
#Save the regression
mod= lm(mainact  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis + data , data = m)
...
#Save the regression
mod= lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis , data = m)
...
g1=ggplot(df, aes(group, predicted, group = x, shape= x, linetype=x, color=x)) + 
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),width = 0.1,
                linetype = "solid",position=position_dodge(width=0.15))+
  geom_text(data = t1, aes(group, predicted, label = round(predicted, 3)), hjust = 1.5, size=4,show.legend = FALSE) +
  geom_text(data = t2, aes(group, predicted, label = round(predicted, 3)), hjust = 1.5, size=4,show.legend  = FALSE) +
  geom_text(data = t3, aes(group, predicted, label = round(predicted, 3)), hjust = -0.5, size=4,show.legend  = FALSE) +
  geom_text(data = t4, aes(group, predicted, label = round(predicted, 3)), hjust = -0.5, size=4,show.legend  = FALSE) +
  scale_color_manual(values=c('black','snow4')) +
  scale_y_continuous(labels=scale)+
  theme_classic()+
  labs(group= "Social origin", shape= "Social origin", linetype= "Social origin",color= "Social origin") +
...
figure2 = annotate_figure(figure2, left = text_grob("Log earnings", 
                                                    color = "black", face = "bold", size = 30, rot = 90))


#Combine the left and right panel 
figure <- ggarrange(figure1,NULL, figure2,widths = c(1, 0.1, 1), nrow=1)
ggsave("Figure_5.pdf", scale=1.35, width = 14, height = 7, dpi= 2300,limitsize = FALSE,units = c("in"))
