...
#Keep people who works 10 hours and more
m=filter(m, workhours>9)

#Run regression seperately for each country
a <-  m %>%
  group_by(cntry) %>%
  do(model1 = tidy(lm(mainact  ~ parents_educ + gndr + age + education*parents_educ + education* crisis + parents_educ*crisis + data, data = .),  conf.int=TRUE)) %>%   ## Same as question
...
#Save the figure
ggsave("Figure_A5.pdf", scale=1.35, width = 10, height = 7, dpi= 2300,limitsize = FALSE,units = c("in"))
