...
z <-  m %>%
  group_by(cntry) %>%
  do(model1 = tidy(lm(income  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis , data = .),  conf.int=TRUE)) %>%   ## Same as question
...
g1= ggplot(data= q,mapping = aes(estimate,cntry)) + 
  geom_point(size=4) +
...
#Save the figure
ggsave("Figure_A3.pdf", scale=1.35, width = 12, height = 7, dpi= 2300,limitsize = FALSE,units = c("in"))
