...
c1 <- m %>% dplyr::select(c("ln_wage", "parents_educ", "gndr", "education", "age", 
...
ols_1 <- c1 %>%
  filter(cntry=="France") 
ols_1 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_1)

ols_2 <- c1 %>%
  filter(cntry=="Germany") 
ols_2 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_2)

ols_3 <- c1 %>%
  filter(cntry=="Italy") 
ols_3 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_3)

ols_4 <- c1 %>%
  filter(cntry=="Poland") 
ols_4 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_4)

ols_5 <- c1 %>%
  filter(cntry=="Spain") 
ols_5 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_5)

ols_6 <- c1 %>%
  filter(cntry=="United Kingdom") 
ols_6 =lm(ln_wage  ~ parents_educ + gndr + age + education*parents_educ + education*crisis + parents_educ*crisis, data=ols_6)
...
cm <- c('(Intercept)' = 'Intercept',
        ...
        'parents_educLow social origin:crisisPost-recession' = 'Parental education low x crisis',
        'parents_educMiddle social origin:crisisPost-recession' = 'Parental education medium x crisis')
...
attr(rows, 'position') <- c(5,10,19,28,33)



# Output Table
modelsummary(results, statistic = 'std.error',  stars = TRUE,
...
modelsummary(results, statistic = 'std.error',  stars = TRUE,
             fmt= '%.3f',coef_map=cm, gof_omit = 'F|R2|AIC|BIC|Log', 
             output = 'table_A3.docx', add_rows = rows)
